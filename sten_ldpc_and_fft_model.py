...
import os 
import ldpc
from encdec_mag_helpers import encode_6_bits, decode_6_bits, MagPredictor
...
def embedd_msg(image, msg):
    mag_predictor = MagPredictor().predict_mag
    ...
    for i in range(0, M, BLOCK_HEIGHT):
        for j in range(0, N, BLOCK_WIDTH):
            ...
            image[i:min(i+BLOCK_HEIGHT,M), j:min(j+BLOCK_WIDTH, N)] = encode_6_bits(image_block, cur_bits, mag_finder=mag_predictor)
...
def extract_msg(image):
    ...
    print("Error: Not enough bits were hidden, only " + str(retreived_bits) + " were.")
    sys.exit(1)
